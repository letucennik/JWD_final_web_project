...
<fmt:message bundle="${loc}" key="clients.title" var="title"/>
...
<fmt:message bundle="${loc}" key="clients.ban" var="ban"/>
            ...
            <tbody>
                <c:forEach var="client" items="${requestScope.allClients}">
                    <tr>
                        ...
                        <td>${client.email}</td>
                        <td><a href="Controller?command=gotoactiveorderspage&userId=${client.id}">${active_orders}</a> </td>
                        <td><a href="Controller?command=viewuserdetails&userId=${client.id}">${info}</a> </td>
                        <td><a onclick="ban()" href="Controller?command=banuser&userId=${client.id}&url=${requestScope.fullUrl}"> ${ban}</a></td>
                    </tr>
                </c:forEach>
            </tbody>
        </table>
